...
### A 실습 운영 확인 — 2026-09-18

규칙셋 **1.2.4**, 새 검증 상품으로 자연어 입력부터 장소 연결·검수·미리보기·확정·자동 재검수까지 직접 진행했습니다. 겹침 30분 + 이동 6분 = 36분 부족이 표시됐고, 오죽헌을 12:00–13:30으로 옮긴 뒤 오류가 **2 → 0**, 준비도가 **76 → 96점**으로 바뀌었습니다. 상품 구성 주의 1건은 그대로 남습니다. 현재 데이터에 따른 실측이며 목표 점수를 보장하는 예시는 아닙니다.

![장소별 방문시간·운영시간·쉬는 날과 이동 구간을 나눈 새 기획 화면](assets/554-plan-cards.png)

*수정 후 기획 화면. 요금·주차는 필요할 때 펼쳐 봅니다.*

![겹침과 이동 부족을 각각 표시하고 이동 여유를 확보하는 수정안](assets/554-overlap-travel.png)

*시간 겹침과 이동 부족이 함께 표시됩니다. 둘 중 하나의 수정안만 선택해 미리 봅니다.*

![수정 후 오류 2건 해소, 준비도 76점에서 96점으로 변화](assets/554-comparison.png)

*상품 구성 주의가 남아 있더라도 이번에 고친 두 오류의 해소를 구분해서 확인합니다.*
