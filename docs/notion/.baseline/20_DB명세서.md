...
<td>text</td>
<td>NOT NULL · CHECK</td>
<td>`KTO` `KTO_WITH` `KTO_PET` `KTO_RELATED` `KTO_DURUNUBI` `KTO_VISITOR` `KAKAO_MOBILITY` `KMA` `LLM`. **공사 서비스는 활용신청 · 하루 한도가 서비스마다 따로라 따로 센다** — `KTO` 국문 관광정보 · `KTO_WITH` 무장애 여행 정보 · `KTO_PET` 반려동물 동반여행정보(`KorPetTourService2`) · `KTO_RELATED` 관광지별 연관 관광지 정보 · `KTO_DURUNUBI` 두루누비 · `KTO_VISITOR` 빅데이터 지역별 방문자수. 예산은 `KTO` 가 `system_setting.daily_quota`, 나머지 다섯은 서비스마다 코드 상수 `EXTRA_SERVICE_RAISED_CAP` — 트래픽 증설로 `KTO_WITH` · `KTO_PET` · `KTO_RELATED` 는 8,000, `KTO_DURUNUBI` · `KTO_VISITOR` 는 800 이다. 증설이 기간제(2026.09.17 ~ 10.16)라 종료일 다음 날부터 다섯 다 800 으로 돌아간다(`extraServiceDailyCap`)</td>
</tr>
<tr>
<td>`operation`</td>
<td>text</td>
