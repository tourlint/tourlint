...
└── packages/shared            사유코드 41종 · 등급 · 표준 시드(R10 63행 · 체류시간 · 실내외) · 기획 · 에이전트 응답 타입
```
...
<tr>
<td>`reasonCode`</td>
<td>사유 코드 41종 중 하나. **화면에 그대로 노출하지 않는다** (G-015 · EX-MS-002)</td>
</tr>
<tr>
<td>`message`</td>
...
<td>계정당 분당 상한 · **같은 에이전트는 계정당 동시 1회**. 예산 100% 에서 검수와 같은 게이트 — 429 `BUDGET_EXHAUSTED`</td>
...
<td>POST</td>
<td>`/api/v1/products/{productId}/items`</td>
<td>일정 항목 추가. 본문 확장 — `afterItemId`(넣을 위치, 없으면 맨 뒤) · `content{contentId, contentTypeId, lcls1, lcls2, lcls3, mapx, mapy}` 면 CONFIRMED · `excluded{label}` 면 EXCLUDED(직접 정한 곳) · `origin`. **시각 자동 채움** — 시작 = 앞 항목 종료 + 이동시간, 잴 수 없으면 앞 항목 종료 시각 그대로. 기존 항목의 시각은 바꾸지 않는다. 식당 · 카페 · 숙소는 식사 · 휴식 · 숙박 유형(숙박은 끝 비움)</td>
...
POST /api/v1/products/{id}/items     기존 { dayNo, start, end, place, itemType } 에 더해
  ...
  { "dayNo": 1, "afterItemId": 17, "itemType": "SIGHT", "origin": "PICKER", "excluded": { "label": "바다부채길" } }
  → EXCLUDED 항목 (걷기 길 · 직접 정한 곳)
```
...
<callout icon="📄" color="gray_bg">
	**가장 최근 검수 실행만 리포트로 만듭니다** (2026.08.29 · F11 구현)
	...
	거절은 409 `REPORT_FAILED` 이며 다시 검수한 뒤 내려받도록 안내합니다. 사유코드 41종에 "최신이 아님" 이 없어 재시도 유도가 붙은 리포트 생성 거절(EX-AU-011)로 묶었습니다.
</callout>
...
<td>두루누비 걷기 길 목록(길이 · 소요 · 난이도 · 좌표). 넣으면 직접 정한 곳</td>
<td>FR-PL-015 · 1콜</td>
</tr>
<tr>
<td>GET</td>
...
<td>POST</td>
<td>`/api/v1/products/{productId}/items`</td>
<td>본문 확장(4-3 참조): `content{…}` 면 CONFIRMED 로 생성, `excluded{label}` 면 EXCLUDED, `afterItemId` 로 자리 지정, 시각 자동 채움, 식당 · 카페 · 숙소는 식사 · 휴식 · 숙박 유형</td>
...
셋 다 실패 · 시간 초과면 `LLM_UNAVAILABLE`, 예산 소진이면 429 `BUDGET_EXHAUSTED`(EX-AG-001). 같은 계정의 같은 에이전트는 동시에 하나만 돈다(FR-AG-002).
**요청 · 응답 모양**
```json
POST /api/v1/products/{id}/place-suggestions   본문 { "itemIds"?: [21, 22] }   (없으면 PENDING 전체)
  → { "items": PlaceSuggestion[], "summary": { "found": 5, "notFound": 1, "noName": 2 } }
...
POST /api/v1/radar/today
  → { "basisAt": "2026-09-11T05:00:00+09:00", "todos": TodayItem[], "quiet": [ { "productId": 9, "text": "태백 당일 산행은 바뀐 정보가 없어요." } ] }
    도구: radar/summary · radar/changes · notifications · 관심 지역 새 소식 · 상품 목록. 공사 0콜 + LLM 1회. 순서는 서버가 정한다(출발일이 가까운 상품의 바뀐 정보 → 새 소식)
모든 에이전트: 사람이 누를 때만 · 계정당 같은 에이전트 동시 1회 · 30초 · 실패 · 시간 초과는 LLM_UNAVAILABLE, 예산 소진은 429 BUDGET_EXHAUSTED. 필드는 더해도 되지만 도구 결과 밖 값은 넣지 않는다
타입 PlaceSuggestion · CheckQuestionPlace · TodayItem 은 packages/shared agent.ts. TodayItem.region 은 { regnCd, signguCd, month } — 시군구 코드(3자리)는 시도 코드와 함께여야 한 곳으로 정해진다
```
<callout icon="🤖" color="blue_bg">
...
<td>FRAGMENT</td>
<td>200</td>
<td>확인 불가. 검수는 계속. 에이전트(4-11)의 실패 · 시간 초과는 새 코드 없이 이 코드를 요청 단위로 쓰고 사람이 하는 길을 둔다 (EX-AG-001)</td>
</tr>
<tr>
<td>`KTO_FETCH_FAILED`</td>
<td>CONTENT</td>
...
<td>REQUEST</td>
<td>429</td>
<td>신규 검수 · 기획 조회 · 에이전트 차단. 기획 화면 안내에는 입력 · 저장은 계속된다는 문구를 더한다</td>
</tr>
<tr color="red_bg">
<td>`FORBIDDEN_ACTION`</td>
<td>REQUEST</td>
