...
<td>알림 목록. `kind=RISK|OPPORTUNITY` · `unread=true`. 행마다 `placeName` — 바뀐 곳 · 새로 생긴 곳의 이름을 **표시 시점에 읽어** 붙입니다(저장하지 않음 · 메모리 10분 캐시 · 한 쪽의 서로 다른 콘텐츠 수만큼 공통정보 1콜). 표출이 중단된 곳(FR-AU-071)과 못 읽은 곳은 `null` 이고, 이름 조회가 실패해도 목록은 그대로 나갑니다</td>
<td>FR-MO-033·035</td>
</tr>
<tr>
<td>POST</td>
