...
<td>smallint</td>
<td>6</td>
<td>R07 연속 일정 기준 시간. **회사 기준 — 표준(6시간)보다 엄격하게만 저장** (CHECK 1–6 · DR-CF-008)</td>
</tr>
<tr>
<td>`r07_meal_minutes`</td>
<td>smallint</td>
<td>60</td>
<td>R07 최소 식사 시간. **회사 기준 — 표준(60분)보다 엄격하게만 저장** (CHECK 60–240 · DR-CF-008)</td>
</tr>
<tr>
<td>`r04_threshold`</td>
<td>smallint</td>
...
<td>text</td>
<td>NOT NULL · CHECK</td>
<td>`KTO` `KTO_WITH` `KTO_RELATED` `KTO_DURUNUBI` `KTO_VISITOR` `KAKAO_MOBILITY` `KMA` `LLM`. **공사 서비스는 활용신청 · 하루 한도가 서비스마다 따로라 따로 센다** — `KTO` 국문 관광정보(반려동물 동반여행정보는 같은 서비스라 여기에 포함) · `KTO_WITH` 무장애 여행 정보 · `KTO_RELATED` 관광지별 연관 관광지 정보 · `KTO_DURUNUBI` 두루누비 · `KTO_VISITOR` 빅데이터 지역별 방문자수. 예산은 `KTO` 가 `system_setting.daily_quota`, 나머지 넷은 서비스마다 코드 상수 `EXTRA_PROVIDER_DAILY_CAP`(800)</td>
</tr>
<tr>
<td>`operation`</td>
<td>text</td>
...
<td>CHECK 제약 `ck_set_span` · `ck_set_meal` **개정** (종전 1–24 · 1–240) + API 거부 `SETTING_NOT_STRICTER`</td>
</tr>
<tr>
<td>FR-MO-059</td>
...
CREATE TABLE user_setting (
    ...
    -- DR-CF-008 : 회사 기준은 표준(6시간 · 60분)보다 엄격하게만
...
CREATE TABLE api_call_log (
    ...
    -- (반려동물 동반여행정보는 국문 관광정보와 같은 서비스라 KTO)
    CONSTRAINT ck_log_provider CHECK (provider IN
        ('KTO','KTO_WITH','KTO_RELATED','KTO_DURUNUBI','KTO_VISITOR','KAKAO_MOBILITY','KMA','LLM')),
...
<td>**더하기만 한다.** `product.planned_at` · `plan_origin` / `itinerary_item.matched_by` · `origin` · `walk_id` · `place_label` NULL 허용과 `ck_item_label_required` · `ck_item_matched_by` · `ck_item_origin` · `ck_item_walk` / `audit_run.setting_snapshot` / `demand_signal.by_keyword` · `ck_signal_type`(T3) / `api_call_log` `ck_log_provider`(서비스별 4값) / `user_setting.watch_regions` · `r07_history` · `updated_at` · `ck_set_span`(1–6) · `ck_set_meal`(60–240). 표 3종과 `weights` · `r04_threshold` 는 남긴다 — 구 코드가 이 DB 에서 그대로 돈다</td>
<td>v2.3 · 릴리즈 1</td>
</tr>
<tr color="red_bg">
<td>`2026-09-xx_drop_account_tables.sql`</td>
<td>**삭제 예정(릴리즈 2).** `target_profile` · `dwell_default` · `indoor_outdoor_map` DROP, `user_setting.weights` · `r04_threshold` DROP. 같은 PR 에서 `EXPECTED_TABLE_COUNT` 를 21 → 18 로 바꾸고 이 문서의 3-11 을 지우며 2-3 을 17종으로 고친다</td>
<td>릴리즈 2 (릴리즈 1 이 하루 이상 정상 가동한 뒤)</td>
</tr>
</table>
<callout color="yellow_bg">
	**릴리즈 1 마이그레이션이 기존 행에 하는 일** — 셋 중 하나라도 빠지면 적용이 실패하거나 화면이 틀어집니다.
	...
	③ CHECK 를 조이기 전에 `r07_span_hours` 는 6 이하로, `r07_meal_minutes` 는 60 이상으로 되돌립니다. 안 하면 `ALTER` 가 실패합니다.
</callout>
## 8-2. 인덱스 생성
```sql
-- DR-IN-011 최소 목록
...
<td>`user_setting` `ck_set_span` · `ck_set_meal` + `audit_run.setting_snapshot`(4-7) · 코드 상수 `STANDARD_VERSION`</td>
</tr>
<tr>
