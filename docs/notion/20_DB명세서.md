...
<td>검수 실행 내 캐시는 **요청 스코프 Map**으로만 구현. 어댑터에 `@Cacheable` 부착 금지. **예외 — 대체 관광지 표시명 (구현 확정 2026.09.10)**: `PlaceNameResolver` 는 `detailCommon2`의 `title` **문자열만** 프로세스 메모리에 `TTL 10분 · 최대 500건`으로 보관한다. 응답 객체를 직렬화하지 않으며 디스크·DB 어디에도 쓰지 않는다. 수정안이 명칭을 저장하지 않으므로(DR-PR-001) 카드를 펼칠 때마다 조회가 나가는데, 요청 스코프로 두면 같은 화면을 다시 열 때마다 예산을 쓴다(NF-CO-002 일일 800건). PDF 5분 보관과 같은 성격의 예외다 (API 설계 4-7)</td>
</tr>
<tr>
<td>알림 본문</td>
