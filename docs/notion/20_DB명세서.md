...
<td>**사용자가 입력한 장소명. 공사 원문이 아님** (무저장 원칙 준수). **`CONFIRMED`**** 항목만 NULL 허용** — 장소 담기로 넣어 칸이 빈 항목은 표시할 때 공식 명칭을 실시간 조회한다(`PlaceNameResolver`). `PENDING` · `EXCLUDED` 는 NOT NULL. 고른 곳의 공사 `title` 을 이 칸에 복사하지 않는다 (DR-IN-013 · PM-NG-014)</td>
</tr>
<tr>
<td>`item_type`</td>
<td>text</td>
...
<td>text</td>
<td>NOT NULL</td>
<td>**두 네임스페이스 공존** — 규칙 판정 사유코드 15종 또는 예외 사유코드 41종 (전체 목록: 예외처리 4장). 화면 미노출, 로그·API 응답 전용 (EX-CM-002 · EX-CM-022 · G-015)</td>
</tr>
<tr>
...
<tr>
<td>DR-IN-013</td>
<td>`place_label`은 `CONFIRMED` 항목에서만 NULL 허용 (표시명은 실시간 조회). `PENDING` · `EXCLUDED`는 NOT NULL</td>
...
<tr>
<td>장소 담기 카드 응답</td>
<td>종류별 목록 · 카드 펼침 응답(`title` · `addr1` · `firstimage` · 이용시간 · 요금 등)을 캐시에 직렬화하거나 로그에 남기는 실수</td>
<td>응답으로만 내려보내고 저장하지 않음. 캐시는 **프로세스 메모리 10분**(`PLAN_LIST_CACHE_TTL_MS`)으로 지역 · 중분류별 `totalCount` · 연 목록 · 무장애 · 반려동물 `contentid` 집합만. 위 「캐시 계층」의 10분 메모리 캐시를 상세 정보까지 넓힐지는 A · B 가 정하며, 넓혀도 디스크 · DB · 로그에는 쓰지 않음</td>
</tr>
<tr>
<td>AI 에이전트 결과</td>
...
CREATE TABLE itinerary_item (
    ...
    -- DR-IN-013 : 장소명이 비어도 되는 것은 CONFIRMED 뿐 (표시명은 실시간 조회)
    CONSTRAINT ck_item_label_required CHECK (match_status = 'CONFIRMED' OR place_label IS NOT NULL),
...
<td>**더하기만 한다.** `product.planned_at` · `plan_origin` / `itinerary_item.matched_by` · `origin` · `place_label` NULL 허용과 `ck_item_label_required` · `ck_item_matched_by` · `ck_item_origin` / `audit_run.setting_snapshot` / `demand_signal.by_keyword` · `ck_signal_type`(T3) / `api_call_log` `ck_log_provider`(서비스별 4값) / `user_setting.watch_regions` · `r07_history` · `updated_at` · `ck_set_span`(1–6) · `ck_set_meal`(60–240). 표 3종과 `weights` · `r04_threshold` 는 남긴다 — 구 코드가 이 DB 에서 그대로 돈다</td>
<td>v2.3 · 릴리즈 1</td>
</tr>
<tr color="red_bg">
