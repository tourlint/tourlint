...
<td>장소 담기로 넣은 두루누비 걷기 길의 식별자. `EXCLUDED` 항목에만 둔다. **코스 이름은 저장하지 않는다** — 표시할 때 상품 지역의 걷기 길 목록(두루누비 1콜 · 10분 메모리 캐시)에서 이 값으로 찾고, 못 찾으면 "걷기 길"로 보인다. `place_label` 에 사용자가 쓴 글이 있으면 그 글을 먼저 보인다. 다시 골라 `CONFIRMED` 가 되면 비운다 (DR-MD-005 · EI-KT-025)</td>
</tr>
<tr>
<td>`created_at`</td>
<td>timestamptz</td>
