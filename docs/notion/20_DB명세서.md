<callout icon="🗄" color="blue_bg">
	# TourLint DB 명세서
	**PostgreSQL 물리 스키마 정의서 — 테이블 20종(로그인 세션·가입 인증 포함) · 제약 · 인덱스 · 실행 가능한 DDL**
...
<callout icon="🔒" color="green_bg">
	**스키마 자체가 설계 원칙의 증거입니다.**
	관광지명·주소·개요·설명·이미지 URL을 담는 컬럼이 전체 20개 테이블에 단 하나도 존재하지 않습니다.
...
<tr>
<td>기본키</td>
<td>일반 엔터티는 `id BIGSERIAL PRIMARY KEY`. 인증용 예외: `session.id` TEXT, `signup_verification.email` TEXT, `auth_email_rate_limit.bucket` TEXT</td>
<td>DR-MD 공통</td>
</tr>
<tr>
<td>생성 시각</td>
<td>일반 엔터티는 `created_at timestamptz NOT NULL DEFAULT now()`. 가입 인증·요청 한도 테이블은 last_sent_at · window_started_at · expires_at 등 목적별 시각으로 보관·만료를 관리한다.</td>
<td>DR-MD 공통</td>
</tr>
<tr>
<td>시각 타입</td>
...
<tr>
<td>스키마 변경</td>
<td>마이그레이션 스크립트로 관리. 정식 배포(2026.08.28) 이후 컬럼 삭제·타입 변경 금지 — 예외는 릴리즈 2 의 `2026-09-16_drop_account_tables.sql` 하나(8장 마이그레이션 표)</td>
<td>NF-DP-009</td>
</tr>
</table>
---
...
account 1 ──< user_setting 1   회사 기준 R07 두 값 · 관심 키워드 · 관심 지역
  (R10 프로파일 · 체류시간 · 실내 · 야외는 계정 표가 아니라 packages/shared 표준 시드)
...
<tr>
<td>20</td>
<td>`auth_email_rate_limit`</td>
<td>가입 인증메일 전역 요청 한도</td>
<td>PM-AC-008</td>
<td>–</td>
</tr>
</table>
기존 도메인 엔터티 17종에 `session` 및 가입 인증용 2종을 포함해 물리 테이블은 **20종**이다. `EXPECTED_TABLE_COUNT`도 20이다. `2026-09-18_signup_verification.sql`을 운영 코드 배포 전에 적용했으며 기존 계정·상품 데이터는 변경하지 않았다.
---
# 3. 테이블별 상세 명세
## 3-1. `account` — 계정
...
<td>text</td>
<td>NULL 허용</td>
<td>타깃 고객. R10 기대 프로파일 조회 키. **표준 키(****`TARGET_KEY`**** 7종)만 허용** — 애플리케이션 검증 (DR-IN-015)</td>
</tr>
<tr>
<td>`concept_key`</td>
<td>text</td>
<td>NULL 허용</td>
<td>상품 콘셉트. R10 조회 키(R04 판정은 콘셉트로 빼지 않는다 · FR-RU-042). **표준 키(****`CONCEPT_KEY`**** 9종)만 허용** — 애플리케이션 검증 (DR-IN-015)</td>
</tr>
<tr>
<td>`head_count`</td>
<td>int</td>
...
<td>timestamptz</td>
<td>NULL 허용</td>
<td>검수 시작을 누른 시각. **NULL = 기획 중.** `PENDING` 항목이 0건일 때만 기록할 수 있다 (DR-IN-014)</td>
</tr>
<tr>
<td>`plan_origin`</td>
<td>jsonb</td>
<td>NULL 허용</td>
<td>기획 출처. 시작 방식 · 신호 종류 · 지역 코드 · 기간 · `contentid` 만 담는다. **원문(행사명 · 제목) 없음** (DR-PR-009)</td>
</tr>
<tr>
<td>`released_at`</td>
<td>timestamptz</td>
...
<td>`place_label`</td>
<td>text</td>
<td>NULL 허용 · CHECK `ck_item_label_required`</td>
<td>**사용자가 입력한 장소명. 공사 원문이 아님** (무저장 원칙 준수). **`CONFIRMED`**** 항목과 걷기 길 항목만 NULL 허용** — 장소 담기로 넣어 칸이 빈 항목은 표시할 때 공식 명칭을 실시간 조회하고(`PlaceNameResolver`), 걷기 길(`walk_id` 가 있는 `EXCLUDED`)은 코스 이름을 두루누비 목록에서 찾는다. 그 밖의 `PENDING` · `EXCLUDED` 는 NOT NULL. 고른 곳의 공사 `title` 과 걷기 길 코스 이름을 이 칸에 복사하지 않는다 (DR-IN-013 · PM-NG-014)</td>
</tr>
<tr>
<td>`item_type`</td>
<td>text</td>
...
<td>`matched_by`</td>
<td>text</td>
<td>NULL 허용 · CHECK `ck_item_matched_by`</td>
<td>장소를 고른 방식. `AUTO` 후보 1건 자동 확정 · `USER` 사람이 후보 목록에서 고름 · `AGENT` 기획 에이전트의 제안을 사람이 선택. 화면 "AI가 찾음" 표시의 출처. 장소 담기 · 레이더 소식으로 넣은 항목은 `origin` 으로 구분한다</td>
</tr>
<tr>
<td>`origin`</td>
<td>text</td>
<td>NULL 허용 · CHECK `ck_item_origin`</td>
<td>항목이 들어온 경로. `MANUAL` 직접 입력 · `UPLOAD` 엑셀 올리기 · `TEXT` 메모 붙여넣기 · `PICKER` 장소 담기 · `SIGNAL` 레이더 소식 · `PATCH` 수정안 적용. NULL = 이 컬럼이 생기기 전 항목</td>
</tr>
<tr>
<td>`walk_id`</td>
<td>text</td>
<td>NULL 허용 · CHECK `ck_item_walk`</td>
<td>장소 담기로 넣은 두루누비 걷기 길의 식별자. `EXCLUDED` 항목에만 둔다. **코스 이름은 저장하지 않는다** — 표시할 때 걷기 길 목록(두루누비 전국 1콜 · 10분 메모리 캐시)에서 이 값으로 찾고, 못 찾으면 "걷기 길"로 보인다. `place_label` 에 사용자가 쓴 글이 있으면 그 글을 먼저 보인다. 다시 골라 `CONFIRMED` 가 되면 비운다 (DR-MD-005 · EI-KT-025)</td>
</tr>
<tr>
<td>`created_at`</td>
<td>timestamptz</td>
...
<callout icon="📍" color="blue_bg">
	**`match_status`**** 상태 전이**
	...
	`PENDING` → ("직접 정한 곳으로 두기" 선택 또는 검수 시작 창의 "이대로 검수 시작") → `EXCLUDED`. 검색 결과가 0건이어도 `PENDING` 으로 남는다 (FR-IN-025)
	장소 담기로 넣은 걷기 길은 처음부터 `EXCLUDED` 이고 `walk_id` 만 가진다. 다시 골라 `CONFIRMED` 가 되면 `walk_id` 를 비운다
...
<td>text</td>
<td>NOT NULL · CHECK</td>
<td>`INITIAL` 검수 시작 직후 · `MANUAL` 다시 검수 · `PATCH` 패치 확정 직후 · `BATCH` 자동 경량 배치 (FR-AU-026)</td>
</tr>
<tr>
<td>`progress_done`</td>
<td>int</td>
...
<tr color="green_bg">
<td>`setting_snapshot`</td>
<td>jsonb</td>
<td>NULL 허용</td>
<td>**산출 시점 적용 기준 스냅샷.** 표준 버전 · 회사 기준 두 값(`{standardVersion, r07SpanHours, r07MealMinutes}`, 4-7). 리포트 머리글 "적용 기준"의 출처이며 소급 변경하지 않는다 (DR-CF-009). NULL = 이 컬럼이 생기기 전 실행</td>
</tr>
<tr>
...
<td>text</td>
<td>NOT NULL</td>
<td>**두 네임스페이스 공존** — 규칙 판정 사유코드 15종 또는 예외 사유코드 43종 (전체 목록: 예외처리 4장). 화면 미노출, 로그·API 응답 전용 (EX-CM-002 · EX-CM-022 · G-015)</td>
</tr>
<tr>
...
<td>timestamptz</td>
<td>NULL 허용</td>
<td>확인 필요 목록의 "확인했어요" 체크 시각. **점수에 미반영**</td>
</tr>
<tr>
<td>`created_at`</td>
<td>timestamptz</td>
...
<td>jsonb</td>
<td>NULL 허용</td>
<td>자체 정규화 산출물 (5장 스키마). 사전 파서가 늘 객체를 만들고, 일부 조각만 읽지 못한 부분 해석은 `unparsed` 배열을 포함한 객체를 그대로 저장한다. 스키마 검증은 AI 응답 경계에서 하며 통과 못 한 LLM 값은 합치지도 저장하지도 않는다 — 앱은 NULL 을 쓰지 않고, 칸은 NULL 허용으로 남긴다 (DR-IN-009)</td>
</tr>
<tr>
...
위에서부터 본다. 비표출 검사가 맨 앞이다 — 데이터 요구사항 6-2(DR-FP) 와 같은 순서.
...
<td>text</td>
<td>NOT NULL</td>
<td>**재노출 판정 키** (FR-MO-036). 동일 조합은 재노출 금지. 조건별로 무엇이 「같은 변경」인지가 달라 앱이 만든다 — `FP:{직전지문|-}:{현재지문}` (조건 1) · `MT:{modifiedtime}` (조건 2·3) · `NEW:{createdtime}` (조건 4 ~ 6) · `CHECKED:{확인한 날}` (페이지 상한을 넘은 날 개별 확인에서 지문이 없는 것 — 표출 중단 등, FR-MO-016)</td>
</tr>
<tr>
<td>`body`</td>
<td>jsonb</td>
...
<tr color="red_bg">
<td>3</td>
<td>여행지(같은 시군구 · 시군구 없는 상품은 같은 시도)에서 열리는 행사의 기간과 여행일이 겹치는 상품</td>
<td>위험</td>
<td>`product(ldong_regn_cd, ldong_signgu_cd)`  • `start_date`  • `nights`</td>
</tr>
<tr>
<td>4</td>
...
계정당 1행이며, 검수 기준 **표준 5 · 회사 기준 2 · 관심 2 · 운영 3** 가운데 계정마다 다른 **회사 기준 2**(R07 두 값)와 레이더에서 등록하는 **관심 2**(관심 키워드 · 관심 지역)가 모이는 지점입니다. 상품별 덮어쓰기는 제공하지 않습니다 (FR-OP-024). 표준 5(가중치 · R04 임계치 · R10 기대 프로파일 · 기본 체류시간 · 실내 · 야외)는 모든 계정에 같아 `packages/shared` 상수로 두고, 운영 3(배치 실행 시각 · 배치 활성화 · 일일 호출 예산)은 계정 단위가 아니라 **전역 설정**이므로 이 테이블이 아니라 `system_setting`(3-11)에 저장합니다.
<table fit-page-width="true" header-row="true">
<tr>
<td>컬럼</td>
<td>타입</td>
...
<td>smallint</td>
<td>6</td>
<td>R07 연속 일정 기준 시간. **회사 기준 — 표준(6시간)보다 엄격하게만 저장** (API 가 거부하고 `ck_set_span` 1–6 이 막는다 · DR-CF-008)</td>
</tr>
<tr>
<td>`r07_meal_minutes`</td>
<td>smallint</td>
<td>60</td>
<td>R07 최소 식사 시간. **회사 기준 — 표준(60분)보다 엄격하게만 저장** (API 가 거부하고 `ck_set_meal` 60–240 이 막는다 · DR-CF-008)</td>
</tr>
<tr>
<td>`watch_keywords`</td>
<td>text 배열</td>
<td>빈 배열</td>
<td>관심 키워드. T1 신호 필터링. 레이더 화면에서 편집한다</td>
</tr>
<tr>
<td>`watch_regions`</td>
<td>jsonb</td>
<td>빈 배열 `'[]'`</td>
<td>관심 지역(시군구 + 달) 배열. 원소 `{regnCd, signguCd, month}`. 배치가 지역마다 T1 · T2(그 달) · T3 를 산출한다 (FR-MO-059 · 3-12). 레이더 화면에서 편집한다</td>
</tr>
<tr>
<td>`r07_history`</td>
<td>jsonb</td>
<td>빈 배열 `'[]'`</td>
<td>회사 기준 변경 이력 **최근 50건**. 원소 `{at, field, from, to}`. 설정은 다음 검수부터 적용되고 과거 결과는 그대로이므로, 지난 리포트와 판정이 다를 때 이유를 여기서 찾는다</td>
</tr>
<tr>
<td>`created_at`</td>
<td>timestamptz</td>
...
<callout icon="⚙️" color="gray_bg">
	R10 기대 프로파일 · 중분류별 기본 체류시간 · 중분류별 실내·야외 매핑은 계정별 테이블이 아니라 **표준 시드**입니다 — `packages/shared` 의 `TARGET_PROFILE_SEED` · `DWELL_MINUTES_SEED` · `INDOOR_OUTDOOR_SEED` 를 모든 계정이 같이 읽고, 사용자는 보기만 합니다. 계정별 테이블 3종은 릴리즈 2 에서 지웠습니다. **배치 실행 시각과 일일 호출 예산(및 배치 활성화)은 서비스 전체에 적용되는 전역 값**으로 `system_setting`(3-11)에 저장합니다 — 계정마다 예산을 가지면 단일 인증키의 물리 한도(개발계정 1일 1,000건)와 합산이 어긋나기 때문입니다 (PM-DA-006).
	**설정 변경은 다음 검수부터 적용되며 과거 ****`audit_run`**** 점수를 소급 변경하지 않습니다** (DR-CF-006). 실행마다 표준 버전과 회사 기준 값을 `audit_run.setting_snapshot` 에 남깁니다 (DR-CF-009).
</callout>
## 3-11. 전역 기준·운영 테이블 5종
...
<td>timestamptz</td>
<td>NOT NULL</td>
<td>수집 시각. 앱이 이 표를 채우지 않는다 — 분류 기준표는 `packages/shared` 상수다 (DR-CF-001)</td>
</tr>
</table>
<callout icon="📚" color="green_bg">
	**분류체계 코드는 무저장 원칙의 대상이 아닙니다.** 관광지 원문이 아니라 **코드 체계 기준표**이기 때문입니다 (DR-CF-001). 기준표(대분류 10 · 중분류 59)는 `packages/shared` 상수(`LCLS_SYSTM1` · `LCLS_SYSTM2`) 한 곳에 두고 규칙 · 화면이 그것을 읽습니다 — 이 표는 앱이 읽지도 쓰지도 않으며, 공사 분류가 바뀌면 상수를 고쳐 배포합니다 (NF-MT-004 · 005).
</callout>
...
<td>text</td>
<td>NOT NULL · CHECK</td>
<td>`KTO` `KTO_WITH` `KTO_PET` `KTO_RELATED` `KTO_DURUNUBI` `KTO_VISITOR` `KAKAO_MOBILITY` `KMA` `LLM`. **공사 서비스는 활용신청 · 하루 한도가 서비스마다 따로라 따로 센다** — `KTO` 국문 관광정보 · `KTO_WITH` 무장애 여행 정보 · `KTO_PET` 반려동물 동반여행정보(`KorPetTourService2`) · `KTO_RELATED` 관광지별 연관 관광지 정보 · `KTO_DURUNUBI` 두루누비 · `KTO_VISITOR` 빅데이터 지역별 방문자수. 예산은 `KTO` 가 `system_setting.daily_quota`(국문 증설이 끝난 2026.10.12 부터는 800 을 넘지 않는다 · `korDailyQuota`), 나머지 다섯은 서비스마다 코드 상수 `EXTRA_SERVICE_RAISED_CAP` — 트래픽 증설로 `KTO_WITH` · `KTO_PET` · `KTO_RELATED` 는 8,000, `KTO_DURUNUBI` · `KTO_VISITOR` 는 800 이다. 증설이 기간제(2026.09.17 ~ 10.16)라 종료일 다음 날부터 다섯 다 800 으로 돌아간다(`extraServiceDailyCap`)</td>
</tr>
<tr>
<td>`operation`</td>
<td>text</td>
<td>NOT NULL</td>
<td>오퍼레이션명 (`searchKeyword2` 등). `LLM` 은 호출 목적 — AI 에이전트 호출도 `provider = 'LLM'` 으로 세고 목적(`PLACE_MATCH` · `CHECK_QUESTIONS` · `TODAY_BRIEF`)으로 나눈다</td>
</tr>
<tr>
<td>`called_at`</td>
<td>timestamptz</td>
...
<td>text</td>
<td>NULL 허용</td>
<td>공사 `resultCode` 등 제공자 결과 코드. 인증 오류 · 한도 초과로 실패한 호출도 공사 코드로 남긴다(앞 0 을 떼고, 한도 초과는 코드가 없어도 `22`) — 오늘 그 제공자에 `22` 실패가 있으면 예산 문이 다 쓴 것으로 본다 (EX-QT-005 · #793)</td>
</tr>
<tr>
<td>`latency_ms`</td>
<td>int</td>
...
<td>boolean</td>
<td>NOT NULL · DEFAULT false</td>
<td>배치 활성화. 개발 기간 중 기본 OFF (NF-DP-007). 심사용 계정을 포함해 사용자 화면에서 바꾸지 않고 운영자만 관리한다 (PM-FN-008 · PM-TA-006)</td>
</tr>
<tr>
<td>`daily_quota`</td>
<td>int</td>
<td>NOT NULL · DEFAULT 8000 · CHECK 1–100000</td>
<td>**서비스 전체** 국문 관광정보 일일 호출 예산. 계정 한도 초과 설정 불가 (DR-IN-008)</td>
</tr>
<tr>
<td>`kor_quota_raised_until`</td>
<td>date</td>
<td>NOT NULL · DEFAULT 2026-10-11</td>
<td>국문 관광정보 트래픽 증설 마지막 날(KST · 포함). 다음 날부터 앱이 `daily_quota` 가 800 을 넘어도 800 으로 쓴다(`korDailyQuota`). 증설이 연장되면 운영자가 `batch_switch.mjs --kor-until` 로 바꾼다 — 배포 없이 (#789 · NF-AV-010)</td>
</tr>
</table>
<callout icon="🌐" color="orange_bg">
	...
	**소진율 = 당일(****`quota_date`**** · KST) 전역 ****`api_call_log`****(provider = KTO) 합산 ÷ ****`system_setting.daily_quota`****.** 배치 실행 시각 · 배치 활성화 · 일일 호출 예산은 사용자 화면에 입력칸을 두지 않고 운영자가 DB(이 표 · 운영 도구 `batch_switch.mjs`)로 관리합니다 (PM-FN-008 · UI-S8-009).
</callout>
## 3-12. 배치 산출 · 캐시 테이블 2종
### `demand_signal` — 수요 신호 T1 · T2 · T3 (배치 산출)
<table fit-page-width="true" header-row="true">
<tr>
<td>컬럼</td>
<td>타입</td>
...
<td>`signal_type`</td>
<td>text</td>
<td>NOT NULL · CHECK `T1` `T2` `T3`</td>
<td>`T1` 최근 30일 신규 등록 · `T2` 행사 밀도 · `T3` 지난해 같은 달 방문자 수 (FR-RU-110 – 122 · FR-MO-059). **T3 는 레이더 전용**이며 빅데이터 지역별 방문자수에서 받은 방문자 수를 `total_count` 에, 지난해 같은 달의 첫날 · 말일(월 창)을 `window_from` · `window_to` 에 담는다. 출처(빅데이터 지역별 방문자수)는 신호 종류로 정해지므로 컬럼을 두지 않고 응답의 `source` 필드로 붙인다</td>
</tr>
<tr color="red_bg">
<td>`region_key`</td>
<td>text</td>
<td>NOT NULL</td>
<td>**`regn`**** 또는 ****`regn:signgu`****.** 시군구가 NULL 인 행이 섞이면 평범한 UNIQUE 가 행마다 다른 것으로 보아 제약이 통째로 논다 — `notification.change_key` 와 같은 함정이다</td>
</tr>
<tr>
<td>`ldong_regn_cd`</td>
<td>text</td>
...
<td>text</td>
<td>NULL 허용</td>
<td>법정동 시군구 코드. 시도 단위 신호는 NULL 이며 그때도 `region_key` 는 채워진다</td>
</tr>
<tr>
<td>`window_from` · `window_to`</td>
<td>date</td>
<td>NOT NULL · CHECK from ≤ to</td>
<td>산출 대상 구간. T1 은 기준일 −30일, T2 는 그 상품의 여행일에서 나온다. 관심 지역(`user_setting.watch_regions`)의 창은 T2 가 그 달, T3 가 지난해 같은 달이다</td>
</tr>
<tr>
<td>`total_count`</td>
<td>int</td>
<td>NOT NULL · CHECK ≥ 0</td>
<td>구간 내 건수. T3 는 관측된 방문자 수(날마다 현지인 · 외지인 · 외국인을 더해 반올림). **강도 점수 · 인기 점수를 담지 않는다** (FR-RU-121 · FR-MO-060)</td>
</tr>
<tr>
<td>`by_type`</td>
<td>jsonb</td>
<td>NOT NULL</td>
<td>`contentTypeId` → 건수 분포. **공사 원문을 담지 않는다** (DR-PR-001)</td>
</tr>
<tr color="red_bg">
<td>`by_keyword`</td>
<td>jsonb</td>
<td>NOT NULL DEFAULT `'{}'`</td>
<td>관심 키워드 → 제목에 그 키워드가 들어간 `contentid` 배열. 배치가 그 창을 쓰는 계정들의 키워드 합집합으로 판정해 지역 단위로 저장하고(계정별 행 없음), 조회는 요청 계정의 키워드로 거른다. 합집합의 키워드는 맞는 곳이 없어도 빈 배열로 남긴다 — 키가 없으면 배치가 아직 안 본 키워드다. **제목을 담지 않는다** — 이름은 화면이 표시할 때 조회한다 (DR-PR-009 · FR-RU-112)</td>
</tr>
<tr>
<td>`computed_at`</td>
<td>timestamptz</td>
<td>NOT NULL</td>
<td>배치 산출 시각. 화면이 언제 센 값인지 표시한다</td>
</tr>
</table>
<callout icon="📡" color="blue_bg">
	**조회가 아니라 배치가 채웁니다** (2026.08.30 결정 · API 설계 4-8)
	T1 · T2 를 조회 시점에 산출하면 레이더 화면을 열 때마다 공사 호출이 나갑니다. `SignalBatchJob`이 변경 감지 배치에 이어 돌면서 지역 · 구간별로 이 표를 채우고, 조회 세 경로는 공사 호출이 **0건**입니다.
	`uq_signal (signal_type, region_key, window_from, window_to)`로 같은 창을 두 번 세지 않습니다 — 상품 열 개가 모두 강릉이면 T1 창이 같습니다. T2 는 여행일에서 나오므로 지역이 같아도 일정이 다르면 창이 다릅니다.
	**산출 전은 행이 없고, 그것은 0 이 아니라 ****`null`****입니다.** 0 은 "세어 보니 없었다"이고 `null`은 "아직 안 세어 봤다"입니다. 화면이 둘을 구분해야 합니다.
	배치는 창 끝이 60일 지난 행을 지웁니다. T3 는 창이 지난해라 그 기준으로 지우면 넣자마자 지워지므로, 그 달 여행이 지난 뒤(창 끝 + 1년이 60일 기준일 전)에 지웁니다.
</callout>
### `llm_parse_cache` — LLM 해석 결과 캐시 (조각 단위)
<table fit-page-width="true" header-row="true">
<tr>
<td>컬럼</td>
<td>타입</td>
...
<td>`fragment_hash`</td>
<td>text</td>
<td>PRIMARY KEY · CHECK `^[0-9a-f]{64}$`</td>
<td>`sha256(purpose + model + fragment)`. **조각 원문 대신 이것만 저장한다** (DR-PR-001 · 6-4)</td>
</tr>
<tr>
<td>`purpose`</td>
<td>text</td>
<td>NOT NULL · CHECK `STRUCTURE` `NORMALIZE`</td>
<td>캐시하는 목적은 구조화 · 정규화 **둘**이다 (EI-LM-001). 구현의 `LlmPurpose` 에 더해지는 AI 에이전트 목적 셋(`PLACE_MATCH` · `CHECK_QUESTIONS` · `TODAY_BRIEF`)은 결과를 저장하지 않으므로 이 표에 들어오지 않는다 (6-4)</td>
</tr>
<tr>
<td>`model`</td>
<td>text</td>
<td>NOT NULL</td>
<td>모델을 바꾸면 답도 달라진다. 키에 섞여 있고 조회 결과에도 남긴다</td>
</tr>
<tr>
<td>`result_json`</td>
<td>jsonb</td>
<td>NOT NULL</td>
<td>해석 **결과**. 조각 원문은 어디에도 남지 않는다</td>
</tr>
</table>
<callout icon="🔁" color="blue_bg">
	**결정론성 때문에 있는 표입니다** (NF-MT-001 · EI-LM-005)
	러너는 매 검수마다 `parseOperatingInfo`를 새로 돕니다. 거기서 LLM 을 직접 부르면 같은 상품을 세 번 검수했을 때 `message`까지 같아야 한다는 약속이 흔들립니다 — `temperature 0`도 매번 같은 답을 보장하지 않습니다. 조각 단위로 답을 붙잡아 두면 두 번째 검수부터 같은 답이 나옵니다.
	**같은 키를 덮지 않습니다** (`ON CONFLICT DO NOTHING`). 덮는 순간 이전 검수와 결과가 갈립니다.
	**실패는 캐시하지 않습니다.** 일시적 장애가 그 조각을 영영 해석 불가로 굳히면 안 됩니다.
</callout>
---
# 4. JSONB 컬럼 스키마
...
```
가중치는 표준 고정(`SEVERITY_WEIGHT_DEFAULT`)이므로 스냅샷은 항상 표준값이지만, 형식은 그대로 유지합니다. `user_setting.weights` 는 릴리즈 2 에서 지웠습니다. 표준 버전과 회사 기준 값은 `setting_snapshot`(4-7)에 따로 남깁니다.
...
<callout icon="🏷" color="red_bg">
	**표시 문구(****`label`****)를 저장하지 않습니다.**
	...
	`TIME_SHIFT` · `REORDER` · `INSERT_ITEM` · `REMOVE_ITEM` — 대상이 사용자의 일정 항목이므로 `itinerary_item.place_label`로 조합합니다 (**0콜**). 장소 담기로 넣어 `place_label` 이 NULL 인 `CONFIRMED` 항목만 `PlaceNameResolver` 로 명칭을 조회합니다 (DR-IN-013 · 6-4 캐시 예외)
</callout>
<callout icon="🎯" color="gray_bg">
...
<callout icon="↩️" color="blue_bg">
	**항목 ****`id`**** 를 담습니다** (2026.08.23 · F09 구현에서 확정).
	...
	걷기 길 항목은 `walkId` 를 함께 담습니다. 되돌린 뒤에도 이 값으로 이름을 다시 찾기 때문이며, 코스 이름은 담지 않습니다 (3-3 `walk_id`).
	`matchedBy` · `origin` 도 함께 담습니다 (2026.09.19 · #586). 되돌리기가 지워진 행을 되살릴 때 「AI가 찾음」 · 「장소 담기에서 넣음」 표시를 잃지 않게 하려는 것입니다. 이 키들이 없는 옛 스냅샷은 `null` 로 읽습니다.
</callout>
...
```json
{
  ...
  "eventPeriod": { "start": "2026-10-31", "end": "2026-10-31" },
...
```
새 소식(조건 4 ~ 6)은 `slot`(넣을 자리 — 일차 · 빈 시간 시작 · 끝 · 분 · 머무는 시간) 또는 `slotMissing`(`NO_GAP` · `DWELL_UNKNOWN`)과, 길찾기로 잰 `precheck` 를 남긴다(UI-S7-008 · FR-MO-052). 시각과 분만이고 이름 · 좌표는 담지 않는다.
<callout icon="🔔" color="red_bg">
	**알림 본문에도 관광지 명칭을 저장하지 않습니다.**
	...
	공사 API 호출 **0콜**이며, 사용자가 직접 입력한 이름으로 보여주므로 오히려 친숙합니다. 장소 담기로 넣어 `place_label` 이 NULL 인 `CONFIRMED` 항목만 `PlaceNameResolver` 로 명칭을 조회합니다 (DR-IN-013 · 6-4 캐시 예외).
...
```
## 4-7. `audit_run.setting_snapshot`
```json
{
  "standardVersion": "2026.09",
  "r07SpanHours": 6,
  "r07MealMinutes": 90
}
```
검수 실행 시점의 표준 버전(코드 상수 `STANDARD_VERSION`)과 회사 기준 두 값(`user_setting.r07_span_hours` · `r07_meal_minutes`)을 담습니다. 리포트 머리글의 "적용 기준 TourLint 표준 2026.09 · 회사 기준 1건(식사 90분)" 같은 문구는 이 값으로 찍으며, 회사 기준을 나중에 바꿔도 과거 실행의 스냅샷은 바꾸지 않습니다 (DR-CF-009). 가중치는 여기가 아니라 `weight_snapshot`(4-1)에 있습니다. 이 컬럼이 생기기 전 실행은 NULL 입니다.
---
...
<tr>
<td>DR-IN-009</td>
<td>`normalized_json`은 5장 스키마 모양. 사전 파서는 늘 객체를 만들고(읽지 못한 조각은 `UNPARSED`), 스키마 검증은 AI 응답 경계에서 한다 — 통과 못 한 LLM 값은 버리고 저장하지 않는다. 앱은 NULL 을 쓰지 않는다</td>
...
<tr>
<td>DR-IN-013</td>
<td>`place_label`은 `CONFIRMED` 항목과 걷기 길(`walk_id` 가 있는 항목)에서만 NULL 허용 (표시명은 실시간 조회). 그 밖의 `PENDING` · `EXCLUDED`는 NOT NULL</td>
<td>CHECK 제약 `ck_item_label_required`</td>
</tr>
<tr>
<td>DR-IN-014</td>
<td>`product.planned_at`은 `PENDING` 항목이 0건일 때만 NOT NULL 이 될 수 있다</td>
<td>애플리케이션 검증 — 검수 시작이 남은 `PENDING` 을 `PLACE_UNRESOLVED` 로 거부하거나, 사용자가 그대로 넘기면 같은 트랜잭션에서 `EXCLUDED` 로 바꾼 뒤 기록</td>
</tr>
<tr>
<td>DR-IN-015</td>
<td>`product.target_key` · `concept_key`는 표준 키 목록 값만 허용</td>
<td>애플리케이션 검증 (마이그레이션이 기존의 표준 밖 값을 NULL 로 비움)</td>
</tr>
<tr>
<td>DR-MD-002</td>
<td>`itinerary_item.origin`은 NULL 또는 `MANUAL` · `UPLOAD` · `TEXT` · `PICKER` · `SIGNAL` · `PATCH`</td>
<td>CHECK 제약 `ck_item_origin`</td>
</tr>
<tr>
<td>DR-MD-003</td>
<td>`itinerary_item.matched_by`는 NULL 또는 `AUTO` · `USER` · `AGENT`</td>
<td>CHECK 제약 `ck_item_matched_by`</td>
</tr>
<tr>
<td>DR-MD-005</td>
<td>`itinerary_item.walk_id`는 `EXCLUDED` 항목에만 둔다</td>
<td>CHECK 제약 `ck_item_walk`</td>
</tr>
<tr>
<td>DR-CF-008</td>
<td>회사 기준은 표준보다 엄격한 값만 저장: `r07_span_hours` 1–6 · `r07_meal_minutes` 60–240</td>
<td>API 거부 `SETTING_NOT_STRICTER`  • CHECK 제약 `ck_set_span` · `ck_set_meal` **개정**(종전 1–24 · 1–240) — CHECK 개정은 릴리즈 2</td>
</tr>
<tr>
<td>FR-MO-059</td>
<td>`demand_signal.signal_type`은 `T1` · `T2` · `T3`</td>
<td>CHECK 제약 `ck_signal_type` **개정** (`T3` 추가)</td>
</tr>
</table>
<callout icon="🛡" color="red_bg">
...
<tr color="blue_bg">
<td>`uq_signal`</td>
<td>`demand_signal(signal_type, region_key, window_from, window_to)` UNIQUE</td>
<td>**\[보완\]** 같은 창을 두 번 세지 않는다. `region_key`가 NOT NULL 이라야 성립한다</td>
</tr>
<tr color="blue_bg">
<td>`idx_signal_lookup`</td>
<td>`demand_signal(signal_type, region_key, window_from)`</td>
<td>**\[보완\]** 레이더 조회 — 산출값 읽기 전용 경로</td>
</tr>
</table>
...
스키마가 깨끗해도 **다른 경로로 공사 원문이 디스크에 남을 수 있습니다.** 아래 9개는 실제로 밟기 쉬운 지점이며 코드 리뷰 체크리스트에 포함합니다.
<table fit-page-width="true" header-row="true">
<tr>
<td>경로</td>
<td>어떻게 새는가</td>
...
<td>검수 실행 내 캐시는 **요청 스코프 Map**으로만 구현. 어댑터에 `@Cacheable` 부착 금지. **예외 — 대체 관광지 표시명 (구현 확정 2026.09.10)**: `PlaceNameResolver` 는 `detailCommon2`의 `title` **문자열만** 프로세스 메모리에 `TTL 10분 · 최대 500건`으로 보관한다. 응답 객체를 직렬화하지 않으며 디스크·DB 어디에도 쓰지 않는다. 수정안이 명칭을 저장하지 않으므로(DR-PR-001) 카드를 펼칠 때마다 조회가 나가는데, 요청 스코프로 두면 같은 화면을 다시 열 때마다 예산을 쓴다(NF-CO-002 일일 800건). PDF 5분 보관과 같은 성격의 예외다 (API 설계 4-7). 이 캐시는 **프로세스에 하나**다 — 상품 상세 · 판정 · 확인 필요 · 미리보기 · 알림 · 장소 정보가 나눠 쓰고, 같은 곳을 동시에 물으면 조회 하나를 같이 기다린다. 한 번에 넷까지 나란히 부르고 2.5초를 넘기면 읽은 데까지만 준다(늦은 조회는 뒤에서 캐시를 채운다). 국문 예산 문을 거쳐 다 쓰면 부르지 않는다 — 화면은 "이름을 불러오지 못한 곳"으로 보인다</td>
</tr>
<tr>
<td>알림 본문</td>
...
<tr>
<td>키워드 일치 결과</td>
<td>"키워드와 맞는 곳"을 보여 주려고 `demand_signal.by_keyword`에 `contentid` 대신 제목을 넣는 실수. 배치 산출값이라 제목 원문이 영구 저장됨</td>
<td>키워드 → `contentid` 배열만 저장하고 이름은 화면이 표시할 때 조회 (3-12 · DR-PR-009). `product.plan_origin`도 시작 방식 · 신호 종류 · 지역 코드 · 기간 · `contentid`만 담고 행사명을 담지 않음</td>
</tr>
<tr>
<td>장소 담기 카드 응답</td>
<td>종류별 목록 · 카드 펼침 응답(`title` · `addr1` · `firstimage` · 이용시간 · 요금 등)을 캐시에 직렬화하거나 로그에 남기는 실수. 걷기 길을 일정에 넣으면서 코스 이름을 `place_label` 에 채우는 실수도 같다</td>
<td>응답으로만 내려보내고 저장하지 않음. 캐시는 **프로세스 메모리 10분**(`PLAN_LIST_CACHE_TTL_MS`)으로 지역 · 중분류별 `totalCount` · 연 목록 · 무장애 · 반려동물 `contentid` 집합만. 위 「캐시 계층」의 10분 메모리 캐시를 상세 정보까지 넓힐지는 A · B 가 정하며, 넓혀도 디스크 · DB · 로그에는 쓰지 않음. 걷기 길은 `walk_id` 만 저장하고 이름은 표시할 때 찾음 (3-3)</td>
</tr>
<tr>
<td>AI 에이전트 결과</td>
<td>에이전트의 도구 결과(검색 · 공통정보 응답) · 제안 문장(이유 · 질문 · 할 일) · 문의처 전화번호를 로그 · 캐시에 남기는 실수</td>
<td>결과는 응답으로만 흐르고 저장 · 로깅하지 않음 (DR-PR-009). 로그에는 에이전트 이름 · 도구 호출 수 · 걸린 시간 · 성공 여부만, `api_call_log`에는 `provider = 'LLM'` · 목적만 남김. 도구가 부르는 공사 조회의 캐시는 위 「캐시 계층」 규칙을 따름</td>
</tr>
</table>
<callout icon="🔍" color="orange_bg">
...
<tr>
<td>기획 중 상품 (`planned_at IS NULL`)</td>
<td>F13 영향 탐색 · 알림 대상에서 제외. 계정당 상품 수(50건까지 성능 보증)에는 센다 — 막는 한도는 없다</td>
<td>DR-LC-008</td>
</tr>
</table>
...
```
**마이그레이션 파일** — 새 DB 는 정본 `db/schema.sql` 을 통째로 적용해 세우고, 이미 만들어진 운영 DB 에는 아래 파일을 순서대로 한 번씩 적용합니다. 파일을 더할 때는 같은 변경을 `db/schema.sql` 과 위 DDL 에도 반영합니다.
<table fit-page-width="true" header-row="true">
<tr>
<td>파일 (`db/migrations/`)</td>
<td>바꾸는 것</td>
<td>명세 버전 · 시점</td>
</tr>
<tr>
<td>`2026-08-27_audit_run_travel_totals.sql`</td>
<td>`audit_run.travel_seconds` · `travel_meters` · `ck_run_travel`</td>
<td>v1.6</td>
</tr>
<tr>
<td>`2026-08-28_notification_change_key.sql`</td>
<td>`notification.change_key` 신설(백필 후 NOT NULL) · `uq_notif_change` 교체</td>
<td>v1.7</td>
</tr>
<tr>
<td>`2026-08-30_demand_signal.sql`</td>
<td>`demand_signal` 신설</td>
<td>v1.9</td>
</tr>
<tr>
<td>`2026-08-30_llm_parse_cache.sql`</td>
<td>`llm_parse_cache` 신설</td>
<td>v2.0</td>
</tr>
<tr>
<td>`2026-09-14_standard_and_plan.sql`</td>
<td>**더하기만 한다.** `product.planned_at` · `plan_origin` / `itinerary_item.matched_by` · `origin` · `walk_id` · `place_label` NULL 허용과 `ck_item_label_required` · `ck_item_matched_by` · `ck_item_origin` · `ck_item_walk` / `audit_run.setting_snapshot` / `demand_signal.by_keyword` · `ck_signal_type`(T3) / `api_call_log` `ck_log_provider`(서비스별 5값) / `user_setting.watch_regions` · `r07_history` · `updated_at`. 표 3종과 `weights` · `r04_threshold`, R07 CHECK(1–24 · 1–240)는 그대로 둔다 — 구 코드가 이 DB 에서 그대로 돈다</td>
<td>v2.3 · 릴리즈 1</td>
</tr>
<tr>
<td>`2026-09-16_drop_account_tables.sql`</td>
<td>`target_profile` · `dwell_default` · `indoor_outdoor_map` DROP, `user_setting.weights` · `r04_threshold` DROP, `r07_span_hours` 를 6 이하로 · `r07_meal_minutes` 를 60 이상으로 되돌린 뒤 `ck_set_span` 1–6 · `ck_set_meal` 60–240 으로 조인다. **코드 배포 뒤에 돌린다** — 지우는 표를 읽는 라우트가 떠 있는 동안 지우면 그 라우트가 500 이다. 되돌릴 수 없으므로 릴리즈 2 가 정상인 것을 보고 돌린다</td>
<td>릴리즈 2</td>
</tr>
<tr>
<td>`2026-09-25_kor_quota_raised_until.sql`</td>
<td>`system_setting.kor_quota_raised_until` 신설(기본 2026-10-11). **더하기만 한다** — 앱은 이 칸이 없는 DB 에서도 기본값으로 읽어 코드 배포와 순서가 어긋나도 된다. 연장을 DB 로 바꾸려면 먼저 적용한다</td>
<td>v2.15</td>
</tr>
</table>
<callout color="yellow_bg">
	**릴리즈 1 마이그레이션이 기존 행에 하는 일** — 둘 다 빠뜨리면 안 됩니다.
	① 검수 실행이 한 번이라도 있는 상품은 `planned_at` 을 `created_at` 으로 채웁니다. 안 하면 기존 상품이 전부 "기획 중"에 놓입니다.
	② `target_key` · `concept_key` 가운데 표준 키가 아닌 값은 NULL 로 비웁니다 (DR-IN-015). 어차피 R10 이 확인 불가로 빠지던 값입니다.
	R07 CHECK 는 릴리즈 1 에서 조이지 않았습니다. 구 설정 API 와 그 테스트가 8시간 · 45분 같은 값을 저장했기 때문입니다. 릴리즈 2 마이그레이션이 기존 행을 표준 안으로 되돌린 **뒤에** 조입니다 — 순서를 바꾸면 기존 행에서 CHECK 가 걸려 마이그레이션 전체가 롤백됩니다.
</callout>
## 8-2. 인덱스 생성
```sql
-- DR-IN-011 최소 목록
...
-- ⚠️ 이메일은 로그인 식별자일 뿐 메일함이 아니다. 콘텐츠랩 제출 '테스트 계정' 기재값
--    (이메일계정 openapi@메일도메인 / 2026openapi!)과 반드시 일치시킨다
INSERT INTO account (email, password_hash, is_demo)
VALUES ('openapi@tourlint.kr', :bcrypt_hash, TRUE);

-- 계정 생성 시 기본값 (DR-CF-002 개정)
--   user_setting        1행 — 회원가입 트랜잭션에서 함께 생성한다. 빈 상태를 허용하지 않는다.
-- R10 기대 프로파일 · 기본 체류시간 · 실내 · 야외는 계정별로 복제하지 않는다.
-- 모든 계정이 packages/shared 표준 시드를 읽는다 (계정별 표 3종은 릴리즈 2 에서 지웠다).
...
<tr>
<td>DR-IN-001–015 무결성</td>
<td>6-1절 제약표 + 8-1·8-3절 DDL</td>
</tr>
<tr>
<td>DR-CF-008–010 검수 기준</td>
<td>`user_setting` `ck_set_span` · `ck_set_meal`(릴리즈 2) + `audit_run.setting_snapshot`(4-7) · 코드 상수 `STANDARD_VERSION`</td>
</tr>
<tr>
<td>DR-PR-009 기획 · 신호 · 에이전트 원문 무저장</td>
<td>`product.plan_origin` · `demand_signal.by_keyword`에 제목 · 행사명 없음 + 6-4절 누출 경로</td>
</tr>
<tr>
<td>DR-LC-001–008 보존</td>
...
<td>수집하지 않기로 했다 — 2026.08.27 실호출(11콜)로 모은 대 10 · 중 59 를 `packages/shared` 상수로 두었다(DR-CF-001). 소분류는 콘텐츠 상세의 값을 그대로 쓴다</td>
<td>끝남</td>
</tr>
<tr>
<td>R09 임계치</td>
...
<callout icon="📌" color="gray_bg">
	**개정 이력**
	...
	v2.0 (2026.08.30) — F03 정규화 폴백에서 확정. **`llm_parse_cache`**** 테이블 신설 — 엔터티 19종 → 20종.** 사전 파서가 못 읽은 조각을 LLM 에 넘기는데, 러너가 매 검수마다 파서를 새로 돌기 때문에 거기서 LLM 을 직접 부르면 NF-MT-001(3회 검수의 `message` 까지 완전 일치)이 흔들린다 — `temperature 0` 도 매번 같은 답을 보장하지 않는다. 조각 단위로 답을 붙잡아 두면 두 번째 검수부터 같은 답이 나온다. 키는 `sha256(purpose + model + fragment)` 이고 값은 해석 **결과**다 — **조각 원문은 저장하지 않는다** (DR-PR-001 · 6-4). 같은 키를 덮지 않는다(`DO NOTHING`) — 덮는 순간 이전 검수와 결과가 갈린다. 실패는 캐시하지 않는다 — 일시적 장애가 그 조각을 영영 해석 불가로 굳히면 안 된다. 마이그레이션 `db/migrations/2026-08-30_llm_parse_cache.sql`.
	v2.2 (2026.09.10) — 6-4 「캐시 계층」 행에 대체 관광지 표시명 예외를 명시. `PlaceNameResolver` 가 `title` 문자열을 프로세스 메모리에 10분·500건 들고 있는데 문서는 「요청 스코프 Map으로만」이라고만 적어 두어 **어느 쪽이 맞는지 알 수 없었다.** 요청 스코프로 바꾸면 수정안 카드를 펼칠 때마다 `detailCommon2` 가 나가 일일 800건을 갉아먹는다 — 수정안이 명칭을 저장하지 않기로 한 결정(DR-PR-001)의 대가다. 응답 객체를 직렬화하지 않고 문자열만 담으므로 이 행이 걱정한 `@Cacheable` 과 모양이 다르다. PDF 5분 보관(API 설계 4-7)과 같은 성격이다. 스키마 변경은 없다. 이슈 #364.
	v2.1 (2026.08.31) — **문서가 스키마를 따라가지 못한 것을 메운다.** v1.6 · v1.9 · v2.0 이 컬럼표와 엔터티 목록만 고치고 실행 DDL 을 안 고쳤다. 문서 머리가 「실행 가능한 DDL」이라 이대로 새 DB 를 세우면 F10 전후 비교(FR-PA-040)와 F14 · F03 이 돌지 않는다. ① 8-1 `audit_run` 에 `travel_seconds` · `travel_meters` 와 `ck_run_travel` 추가 — 둘은 항상 같이 NULL 이거나 같이 값이다 ② **3-13 신설** — `demand_signal` · `llm_parse_cache` 의 컬럼 상세. 종전에는 2-3 엔터티 목록 행 두 개와 개정 이력 문장만 있고 컬럼이 무엇인지 적힌 데가 없었다 ③ 8-1 에 두 표의 `CREATE TABLE`, 8-2 와 6-2 에 `idx_signal_lookup` · `uq_signal` 추가. 스키마 변경은 없다 — `db/schema.sql` 과 마이그레이션 4개가 정본이고 이 문서를 그것에 맞춘 것이다. 연쇄로 16_데이터요구사항 v1.7(3-2 목록 · 4장 DDL)과 21_API 4-7(엔터티 19종 → 20종)을 함께 고쳤다.
	v2.3 (2026.09.15) — 설정 탭 개편안과 기획 · 검수 · 레이더 개편안 v7.1 반영. **릴리즈 1 은 컬럼 · 제약을 더하기만 하고 테이블은 지우지 않는다.** ① `product.planned_at`(NULL = 기획 중) · `plan_origin`(시작 방식 · 신호 종류 · 지역 코드 · 기간 · contentid 만) 신설 ② `itinerary_item.place_label` 을 `CONFIRMED` 항목에서만 NULL 허용(`ck_item_label_required`) — 장소 담기로 넣은 항목의 칸을 공사 `title` 로 채우면 원문 저장이 된다. `matched_by`(AUTO · USER · AGENT) · `origin`(MANUAL · UPLOAD · TEXT · PICKER · SIGNAL · PATCH)과 각 CHECK 신설 ③ `audit_run.setting_snapshot`(표준 버전 · 회사 기준 두 값) 신설, 4-7 에 형태 ④ `user_setting` 회사 기준 CHECK 를 1–24시간 · 1–240분에서 1–6시간 · 60–240분으로 조이고 `watch_regions` · `r07_history`(최근 50건) · `updated_at` 신설. `weights` · `r04_threshold` 는 표준 고정이라 엔진이 읽지 않는다 ⑤ `demand_signal` 에 `T3`(지난해 같은 달 방문자 수) · `by_keyword`(키워드 → contentid, 제목 없음) ⑥ `api_call_log.provider` 에 `KTO_WITH` · `KTO_RELATED` · `KTO_DURUNUBI` · `KTO_VISITOR` — 공사 새 서비스는 활용신청 · 하루 한도가 서비스마다 따로다. 에이전트의 LLM 호출은 `LLM` 으로 세고 목적으로 나눈다 ⑦ 6-4 누출 경로 6개 → 9개(키워드 일치 결과 · 장소 담기 카드 응답 · AI 에이전트 결과) ⑧ 8-1 에 마이그레이션 파일 목록, 마이그레이션 `db/migrations/2026-09-14_standard_and_plan.sql`. **표 3종(****`target_profile`**** · ****`dwell_default`**** · ****`indoor_outdoor_map`****)과 ****`weights`**** · ****`r04_threshold`**** 는 삭제 예정(릴리즈 2)** — 구 코드가 같은 DB 에서 돌아야 롤백할 수 있어 릴리즈 1 이 하루 이상 정상 가동한 뒤 지운다. 그때 3-11 을 지우고 2-3 을 17종으로, 코드 `EXPECTED_TABLE_COUNT` 를 21 → 18 로 고친다. 같은 흐름으로 2-1 · 2-2 의 표 3종 표기, 3-12 `system_setting` 의 화면 문구(운영자 전용 · PM-FN-008), 3-13 `llm_parse_cache.purpose` 설명, 7 의 기획 중 상품 행(DR-LC-008), 8-4 계정 생성 기본값(표 3종 복제 안 함 · DR-CF-002 개정), 9 추적표를 맞췄다. `place_label` 이 NULL 인 항목의 표시명 조회를 4-3 · 4-5 콜아웃에 적고, 1-1 · 9 의 "17개 테이블"을 20개(릴리즈 2 뒤 17개)로 바로잡았다. 연쇄 개정: 16_데이터요구사항 v1.9(DR-MD-001–004 · DR-IN-013–015 · DR-CF-002 개정 · DR-CF-008–010 · DR-PR-009 · DR-LC-008) · 13_기능요구사항 v2.3(FR-OP-021–023 · FR-MO-059–061) · 18_외부연동요구사항 v1.8(EI-KT-001 · EI-LM-001) · 21_API백엔드설계 v2.4. 통합 점검 — 예외 사유코드 41종, `trigger_type` 설명(검수 시작 · 다시 검수), "확인했어요" 체크 시각 문구를 맞췄다. 결정 반영(2026.09.15) — 걷기 길은 코스 이름을 저장하지 않고 `walk_id` 만 남긴다. 3-3 에 컬럼과 `ck_item_walk` 를 더하고 `ck_item_label_required` 를 걷기 길까지 넓혔다(8-1 DDL · 6-1 DR-MD-005 · 마이그레이션 목록 · 4-4 스냅샷 `walkId` · 6-4). 3-3 상태 전이 콜아웃의 「검색 0건 → EXCLUDED」는 FR-IN-025 에 맞춰 고쳤다. 예외 사유코드는 `RATE_LIMIT_EXCEEDED` 가 더해져 42종. `walk_id` 설명 — 이름은 두루누비 전국 목록에서 찾는다(코스 목록에 지역 조건이 없다, 2026.09.15 실호출). 게이트 S2(2026.09.15) — 반려동물 동반여행정보를 `KTO_PET` 으로 따로 센다(3-12 · 8-1 · 마이그레이션 목록, `ck_log_provider` 9값). R07 CHECK 조이기는 릴리즈 2 로 옮겼다 — 구 설정 API 와 그 테스트가 8시간 · 45분 같은 값을 저장해, 릴리즈 1 에서 조이면 구 코드가 이 DB 에서 실패한다(3-10 · 8-1 · 마이그레이션 목록 · 추적 행). 레이더 키워드 일치 구현(2026.09.15) — 3-13 `by_keyword` 에 맞는 곳이 없는 키워드는 빈 배열로 남기고 키가 없으면 아직 안 본 키워드라는 것을 더했다. 관심 지역 신호 구현(2026.09.15) — 3-13 에 T3 합산 방식과 T3 행의 보관 기준(창 끝 + 1년)을 더했다.
	v2.7 (2026.09.17) — `api_call_log.provider` 설명의 예산 상수를 서비스별 표로 고쳤다. 트래픽 증설로 `KTO_WITH` · `KTO_PET` · `KTO_RELATED` 의 하루 상한이 8,000 이 되고 `KTO_DURUNUBI` · `KTO_VISITOR` 는 800 그대로다 — 종전에는 다섯이 한 상수(`EXTRA_PROVIDER_DAILY_CAP` 800)를 썼다. 증설이 기간제(2026.09.17 ~ 10.16)여서 종료일 다음 날부터 다섯 다 800 으로 돌아간다(`extraServiceDailyCap`). 스키마 변경은 없다 — `ck_log_provider` 9값 그대로다. 외부 연동 요구사항 v1.13 · API 설계 v2.10 과 동시 개정. 이슈 #508.
	v2.6 (2026.09.17) — R01 판정 근거 예시의 날짜와 요일이 어긋난 것을 고쳤다(이슈 #433). `visitDate` 가 2026-10-13 인데 `visitWeekday` 가 `MON` 이었다 — 그 날은 화요일이다. 요일 쪽은 예시 서술(「매주 월요일 휴관」)과 맞으므로 날짜를 실제 월요일인 **2026-10-12** 로 옮겼다. API 설계 5-6 의 같은 예시와 동시 개정. **머리표 버전을 함께 맞췄다** — 릴리즈 2 개정(v2.5 · 2026.09.16)에서 머리표를 안 올려 v2.3 으로 남아 있었다. 개정 이력에 `v2.4` 가 없는 것은 번호를 건너뛴 것이며, 마이그레이션 표가 `v2.3` 을 참조하고 있어 소급 재정렬은 하지 않는다. 스키마 변경은 없다.
	v2.5 (2026.09.16) — 릴리즈 2 반영. 계정별 기준표 3종(`target_profile` · `dwell_default` · `indoor_outdoor_map`)과 `user_setting.weights` · `r04_threshold` 를 지웠고, R07 CHECK 를 `ck_set_span` 1–6 · `ck_set_meal` 60–240 으로 조였다 — 기존 행을 표준 안으로 되돌린 뒤에 조인다(순서를 바꾸면 롤백된다). 3-11 을 지우고 3-12 · 3-13 을 3-11 · 3-12 로 당겼으며, 2-3 을 17종으로, 1장 머리를 17종으로, 2-1 ERD · 2-2 트리에서 표 3종을 뺐다. 8-1 DDL 과 8-4 초기 데이터, 마이그레이션 목록(`2026-09-16_drop_account_tables.sql`)도 같이 고쳤다. 코드 `EXPECTED_TABLE_COUNT` 21 → 18. **이 마이그레이션은 코드 배포 뒤에 돌린다** — 지우는 표를 읽는 라우트(`settings/dwell` · `indoor-outdoor` · `profiles` · `lcls`)가 떠 있는 동안 지우면 그 넷이 500 이고, 되돌릴 수 없으므로 릴리즈 2 가 정상인 것을 보고 돌린다.
</callout>
<callout icon="©️" color="gray_bg">
	출처: ⓒ한국관광공사
</callout>
## 2026.09.18 가입 인증 저장소
`signup_verification`은 이메일당 1행으로 verification_id(UUID), code_hash(scrypt, 원문 미저장), expires_at, attempts(0~5), delivered, last_sent_at, window_started_at, send_count를 저장한다. 계정 생성과 코드 소비는 하나의 트랜잭션에서 처리한다. 재전송은 이전 코드를 무효화한다. `auth_email_rate_limit`은 bucket(UTC 분·일), count, expires_at만 저장하며 IP 등 추가 개인정보는 수집하지 않는다. 마지막 발송 후 24시간이 지난 인증 행과 만료 버킷은 주기적으로 삭제한다. 계정 테이블 변경·기존 계정 재인증은 없다. 물리 테이블 수는 기존 18종에서 20종으로 증가한다.
<callout color="gray_bg">
	v2.8 (2026.09.18) — 사용자 요청 #539에 따라 신규 가입 이메일 인증을 필수화. FR-CM-001 · UI-S0-002 · PM-AC-008 · DB · API 연쇄 개정. 기존 계정 및 심사용 계정 로그인 유지.
</callout>
### 가입 인증 DDL
운영 증분 적용 파일: `db/migrations/2026-09-18_signup_verification.sql`. 전체 스키마의 동일 정의는 다음과 같다.
```sql
...
```
<callout icon="📝" color="gray_bg">
	v2.9 (2026.09.18) — 물리 테이블 전체 목록을 session·가입 인증까지 20종으로 통일하고 미적용처럼 남아 있던 이전 릴리즈 설명을 정정. 인증 테이블의 키·시각 예외와 실행 가능한 DDL·마이그레이션 경로를 보완. API·외부 연동 연쇄 정리.
	v2.10 (2026.09.19) — #586: 4-4 스냅샷 항목에 `matchedBy` · `origin` 추가. `walkId` 는 v2.x 에서 이미 담기로 했으나 코드가 담지 않아, 걷기 길이 든 상품의 수정안 확정이 `ck_item_label_required` 로 실패했다(코드를 문서에 맞춤). 스키마 변경 없음.
	v2.20 (2026.09.27) — #908: 6-4 캐시 표의 대체 관광지 표시명 행에 이 캐시는 프로세스에 하나이고(상품 상세 · 판정 · 확인 필요 · 미리보기 · 알림 · 장소 정보가 나눠 씀) 같은 곳을 동시에 물으면 조회 하나를 같이 기다리며, 넷까지 나란히 · 2.5초 상한 · 국문 예산 문을 거친다는 것을 적었다. 화면마다 캐시가 따로여서 결과 화면을 한 번 열 때 같은 곳을 최대 세 번, 한 곳씩 차례로, 예산 문 없이 불렀다.
	v2.19 (2026.09.27) — #863: 4-5 `notification.body` 에 새 소식의 넣을 자리(`slot` · `slotMissing`)와 길찾기 사전 확인(`precheck`)을 더했다 — 시각과 분만이고 이름 · 좌표는 담지 않는다. 배치가 빈 시간과 우회거리를 계산하고도 알림에 남기지 않았다. 스키마 변경은 없다.
	v2.18 (2026.09.27) — #890: 설명을 지금 동작에 맞췄다 — `lcls_systm_code` 는 앱이 읽지도 쓰지도 않고 분류 기준표는 `packages/shared` 상수(2-2 · 2-3 · 3-11 · 10장 「끝남」), 앱은 `normalized_json` 에 NULL 을 쓰지 않음(3-7 · 6-1 DR-IN-009), `concept_key` 로 R04 를 빼지 않음(3-2), 파괴적 변경 예외는 `2026-09-16_drop_account_tables.sql` 하나(1-2), 운영 값은 DB · 운영 도구 `batch_switch.mjs` 로만(3-11 `system_setting`), 기획 중 상품의 50건은 성능 보증 수(7장). 스키마 변경은 없다. 8장 DDL 주석은 `db/schema.sql` 과 글자까지 같아야 해 두었다.
	v2.17 (2026.09.26) — #866: 3-9 `notification.change_key` 에 조건 4 ~ 6 의 `NEW:{createdtime}` 과 페이지 상한을 넘은 날 개별 확인의 `CHECKED:{확인한 날}` 을 더하고, 3-7 변경 감지 판정 로직의 순서를 비표출 검사가 맨 앞이 되게 고쳤다. 이 블록만 데이터 요구사항 6-2 가 08-23 에 고치기 전 순서로 남아 있었다. 스키마 변경은 없다.
	v2.16 (2026.09.25) — #793: 3-16 `api_call_log.result_code` 설명에 인증 오류 · 한도 초과 코드를 남긴다는 것과 예산 문이 `22` 를 읽는다는 것을 적었다. 전에는 이 두 오류의 코드가 비어 「오늘 공사가 한도 초과라고 답했다」 를 알 수 없었다. 스키마 변경은 없다.
	v2.15 (2026.09.25) — #789: 3-12 `system_setting` 에 `kor_quota_raised_until`(국문 증설 마지막 날 · 기본 2026-10-11)을 더했다. 증설이 연장되면 코드 배포 없이 이 값만 바꾼다 — 배포 금지 기간(10.01 – 11.05) 안에 연장될 수 있어서다. 3-12 표 · 8-1 DDL 의 `daily_quota` 기본값이 800 에 멈춰 있어 스키마(8,000 · 2026-09-17)에 맞췄다. 마이그레이션 `db/migrations/2026-09-25_kor_quota_raised_until.sql`. 테이블 수는 그대로다.
	v2.14 (2026.09.25) — #777: `api_call_log.provider` 설명의 `KTO` 예산에 국문 증설 종료(2026.10.11)를 적었다. 10.12 부터 DB 값이 800 을 넘어도 800 이다. 스키마 변경은 없다.
	v2.13 (2026.09.21) — #703: 4-5 `notification.body` 에 `eventPeriod`(행사 알림 · 읽어 낸 날짜)를 더했다. 조건 3 을 건 근거인데 남기지 않아 카드가 「행사 정보가 바뀌었습니다」 밖에 못 말했다. 원문(`20261031`)이 아니라 날짜이고 이름 · 주소는 여전히 남기지 않는다. 스키마 변경은 없다(JSONB).
	v2.12 (2026.09.21) — #689: 영향 탐색 조건 3 의 설명과 참조 컬럼에 지역을 더했다(기능 요구사항 FR-MO-030 v2.18). 스키마 변경은 없다.
	v2.11 (2026.09.20) — #612: `finding.reason_code` 의 예외 사유코드 건수를 43종으로 고쳤다(`INPUT_INVALID` 신설, 예외처리 요구사항 v1.6). 스키마 변경 없음.
</callout>
