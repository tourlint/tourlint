...
<td>관심 키워드 → 제목에 그 키워드가 들어간 `contentid` 배열. 배치가 그 창을 쓰는 계정들의 키워드 합집합으로 판정해 지역 단위로 저장하고(계정별 행 없음), 조회는 요청 계정의 키워드로 거른다. **제목을 담지 않는다** — 이름은 화면이 표시할 때 조회한다 (DR-PR-009 · FR-RU-112)</td>
</tr>
<tr>
<td>`computed_at`</td>
<td>timestamptz</td>
