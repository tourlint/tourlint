<callout icon="🗄" color="blue_bg">
	# TourLint DB 명세서
	**PostgreSQL 물리 스키마 정의서 — 테이블 20종 · 제약 · 인덱스 · 실행 가능한 DDL**
...
<callout icon="🔒" color="green_bg">
	**스키마 자체가 설계 원칙의 증거입니다.**
	관광지명·주소·개요·설명·이미지 URL을 담는 컬럼이 전체 20개 테이블(릴리즈 2 삭제 뒤 17개)에 단 하나도 존재하지 않습니다.
...
erDiagram
    ...
    account ||--o{ target_profile : "R10 프로파일 · 삭제 예정(릴리즈 2)"
    account ||--o{ dwell_default : "기본 체류시간 · 삭제 예정(릴리즈 2)"
    account ||--o{ indoor_outdoor_map : "실내외 매핑 · 삭제 예정(릴리즈 2)"
...
account 1 ──< user_setting 1
          ├──< target_profile        R10 기대 프로파일        삭제 예정(릴리즈 2)
          ├──< dwell_default         중분류별 기본 체류시간   삭제 예정(릴리즈 2)
          └──< indoor_outdoor_map    중분류별 실내 · 야외     삭제 예정(릴리즈 2)
...
<td>R10 기대 프로파일 · **삭제 예정(릴리즈 2)**</td>
<td>없음 — R10 은 표준 시드 `TARGET_PROFILE_SEED` 를 읽는다</td>
<td>–</td>
</tr>
<tr>
<td>12</td>
<td>`dwell_default`</td>
<td>중분류별 기본 체류시간 · **삭제 예정(릴리즈 2)**</td>
<td>없음 — FR-IN-011 · R03 은 표준 시드 `DWELL_MINUTES_SEED` 를 읽는다</td>
<td>–</td>
</tr>
<tr>
<td>13</td>
<td>`indoor_outdoor_map`</td>
<td>중분류별 실내·야외 · **삭제 예정(릴리즈 2)**</td>
<td>없음 — R09 는 표준 시드 `INDOOR_OUTDOOR_SEED` 를 읽는다</td>
<td>–</td>
</tr>
<tr>
<td>14</td>
...
계정당 1행이며, 검수 기준 **표준 5 · 회사 기준 2 · 관심 2 · 운영 3** 가운데 계정마다 다른 **회사 기준 2**(R07 두 값)와 레이더에서 등록하는 **관심 2**(관심 키워드 · 관심 지역)가 모이는 지점입니다. 상품별 덮어쓰기는 제공하지 않습니다 (FR-OP-024). 표준 5(가중치 · R04 임계치 · R10 기대 프로파일 · 기본 체류시간 · 실내 · 야외)는 모든 계정에 같아 `packages/shared` 상수로 두고, 운영 3(배치 실행 시각 · 배치 활성화 · 일일 호출 예산)은 계정 단위가 아니라 **전역 설정**이므로 이 테이블이 아니라 `system_setting`(3-12)에 저장합니다.
<table fit-page-width="true" header-row="true">
<tr>
<td>컬럼</td>
<td>타입</td>
...
<td>출시 준비도 가중치. **표준 고정 — 엔진이 읽지 않음, 삭제 예정(릴리즈 2).** 엔진은 `SEVERITY_WEIGHT_DEFAULT` 를 쓴다</td>
</tr>
<tr>
<td>`r07_span_hours`</td>
<td>smallint</td>
<td>6</td>
<td>R07 연속 일정 기준 시간. **회사 기준 — 표준(6시간)보다 엄격하게만 저장** (API 가 거부, CHECK 1–6 은 릴리즈 2 · DR-CF-008)</td>
</tr>
<tr>
<td>`r07_meal_minutes`</td>
<td>smallint</td>
<td>60</td>
<td>R07 최소 식사 시간. **회사 기준 — 표준(60분)보다 엄격하게만 저장** (API 가 거부, CHECK 60–240 은 릴리즈 2 · DR-CF-008)</td>
</tr>
<tr>
<td>`r04_threshold`</td>
<td>smallint</td>
<td>3</td>
<td>R04 콘텐츠 편중 임계치 (동일 유형 N곳). **표준 고정 — 엔진이 읽지 않음, 삭제 예정(릴리즈 2).** 엔진은 `SETTING_DEFAULTS` 를 쓴다</td>
</tr>
<tr>
...
<td>jsonb</td>
<td>빈 배열 `'[]'`</td>
<td>관심 지역(시군구 + 달) 배열. 원소 `{regnCd, signguCd, month}`. 배치가 지역마다 T1 · T2(그 달) · T3 를 산출한다 (FR-MO-059 · 3-13). 레이더 화면에서 편집한다</td>
</tr>
<tr>
<td>`r07_history`</td>
<td>jsonb</td>
...
<callout icon="⚙️" color="gray_bg">
	R10 기대 프로파일 · 중분류별 기본 체류시간 · 중분류별 실내·야외 매핑은 계정별 테이블이 아니라 **표준 시드**입니다 — `packages/shared` 의 `TARGET_PROFILE_SEED` · `DWELL_MINUTES_SEED` · `INDOOR_OUTDOOR_SEED` 를 모든 계정이 같이 읽고, 사용자는 보기만 합니다. 3-11 의 계정별 테이블 3종은 릴리즈 1 부터 읽지 않으며 삭제 예정(릴리즈 2)입니다. **배치 실행 시각과 일일 호출 예산(및 배치 활성화)은 서비스 전체에 적용되는 전역 값**으로 `system_setting`(3-12)에 저장합니다 — 계정마다 예산을 가지면 단일 인증키의 물리 한도(개발계정 1일 1,000건)와 합산이 어긋나기 때문입니다 (PM-DA-006).
...
## 3-11. 계정 단위 기준 테이블 3종 — 삭제 예정(릴리즈 2)
<callout color="red_bg">
	**릴리즈 1 부터 엔진 · 화면이 이 세 테이블을 읽지 않습니다. 릴리즈 2 에서 DROP 합니다.**
	R10 기대 프로파일 · 기본 체류시간 · 실내 · 야외는 모든 계정에 같은 표준이며, 판정과 검수 기준 탭은 `packages/shared` 의 표준 시드(`TARGET_PROFILE_SEED` · `DWELL_MINUTES_SEED` · `INDOOR_OUTDOOR_SEED`)를 읽습니다. 테이블은 구 코드가 같은 DB 에서 돌 수 있도록 릴리즈 1 에는 남겨 두고, 릴리즈 1 이 하루 이상 정상 가동한 뒤 `db/migrations/2026-09-xx_drop_account_tables.sql` 로 지웁니다(8-1). 회원가입 때 이 세 테이블에 기본값을 복제하지도 않습니다(DR-CF-002 개정) — 아래 본문의 "복제 생성" 문장은 개정 전 기록입니다. 본문은 DROP 전까지의 실제 스키마 기록으로 남기고, DROP 할 때 이 절을 지우고 2-3 을 17종으로 고칩니다.
</callout>
...
```
가중치는 표준 고정(`SEVERITY_WEIGHT_DEFAULT`)이므로 스냅샷은 항상 표준값이지만, 형식은 그대로 유지합니다. `user_setting.weights` 는 엔진이 읽지 않으며 삭제 예정(릴리즈 2)입니다. 표준 버전과 회사 기준 값은 `setting_snapshot`(4-7)에 따로 남깁니다.
...
<td>키워드 → `contentid` 배열만 저장하고 이름은 화면이 표시할 때 조회 (3-13 · DR-PR-009). `product.plan_origin`도 시작 방식 · 신호 종류 · 지역 코드 · 기간 · `contentid`만 담고 행사명을 담지 않음</td>
</tr>
<tr>
<td>장소 담기 카드 응답</td>
...
-- ---------------------------------------------------------------------
-- 11~13. 계정 단위 기준 테이블 — 삭제 예정(릴리즈 2)
--        릴리즈 1 부터 엔진 · 화면이 읽지 않는다 (표준 시드를 읽는다, 3-11)
-- ---------------------------------------------------------------------
...
<tr color="red_bg">
<td>`2026-09-xx_drop_account_tables.sql`</td>
<td>**삭제 예정(릴리즈 2).** `target_profile` · `dwell_default` · `indoor_outdoor_map` DROP, `user_setting.weights` · `r04_threshold` DROP, `r07_span_hours` 를 6 이하로 · `r07_meal_minutes` 를 60 이상으로 되돌린 뒤 `ck_set_span` 1–6 · `ck_set_meal` 60–240 으로 조인다. 같은 PR 에서 `EXPECTED_TABLE_COUNT` 를 21 → 18 로 바꾸고 이 문서의 3-11 을 지우며 2-3 을 17종으로 고친다</td>
<td>릴리즈 2 (릴리즈 1 이 하루 이상 정상 가동한 뒤)</td>
</tr>
</table>
<callout color="yellow_bg">
	**릴리즈 1 마이그레이션이 기존 행에 하는 일** — 둘 다 빠뜨리면 안 됩니다.
	...
	R07 CHECK 는 릴리즈 1 에서 조이지 않습니다. 구 설정 API 와 그 테스트가 8시간 · 45분 같은 값을 저장하므로, 조이면 구 코드가 이 DB 에서 실패합니다. 릴리즈 2 마이그레이션이 기존 행을 표준 안으로 되돌린 뒤 조입니다.
</callout>
## 8-2. 인덱스 생성
```sql
-- DR-IN-011 최소 목록
...
-- 모든 계정이 packages/shared 표준 시드를 읽는다 (target_profile · dwell_default ·
-- indoor_outdoor_map 은 삭제 예정(릴리즈 2), 3-11).
...
<td>20개 테이블(릴리즈 2 뒤 17개)에 명칭·주소·개요·이미지 컬럼 0건. 6-3절 금지 목록</td>
</tr>
<tr>
