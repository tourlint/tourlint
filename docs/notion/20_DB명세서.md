...
<callout icon="🌐" color="orange_bg">
	...
	**소진율 = 당일(****`quota_date`**** · KST) 전역 ****`api_call_log`****(provider = KTO) 합산 ÷ ****`system_setting.daily_quota`****.** 배치 실행 시각 · 배치 활성화 · 일일 호출 예산은 사용자 화면에 입력칸을 두지 않고 운영자가 환경변수 · DB 로 관리합니다 (PM-FN-008 · UI-S8-009).
</callout>
