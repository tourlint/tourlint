<callout icon="🗄" color="blue_bg">
	# TourLint DB 명세서
	**PostgreSQL 물리 스키마 정의서 — 테이블 17종 · 제약 · 인덱스 · 실행 가능한 DDL**
...
11–13 은 릴리즈 1 부터 엔진 · 화면이 읽지 않으며 릴리즈 2 에서 DROP 합니다(3-11). 삭제 후 엔터티는 **17종**입니다. 코드의 기대 테이블 수 `EXPECTED_TABLE_COUNT`(`apps/api/src/health/health.controller.ts`)는 이 목록에 없는 `session` 까지 세어 지금 21 이며, 삭제하는 PR 에서 **21 → 18** 로 함께 바꿉니다 — 안 바꾸면 `/health` 가 `schema: missing` 을 냅니다.
---
# 3. 테이블별 상세 명세
## 3-1. `account` — 계정
