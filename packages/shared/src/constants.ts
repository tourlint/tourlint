 ...
 *    **없는 시간 중복**이 만들어진다.
 *
 * 픽스처 22건이 실제로 쓰는 8개만 담는다. 중분류 기준표(`LCLS_SYSTM2`)가 59행이므로
 * 나머지는 그 표를 기준으로 채운다 — 미매핑은 `SETTING_DEFAULTS.dwellFallbackMinutes`.
 *
 * 주석의 이름은 **공사가 주는 중분류 이름 그대로**다. 임의로 줄여 적으면 표를 넓힐 때
 * 엉뚱한 코드에 값을 넣게 된다.
 */
export const DWELL_MINUTES_SEED: Readonly<Record<string, number>> = {
  HS01: 60,  // 역사유적지 — 경포대 · 굴산사지
  VE03: 60,  // 도시공원 — 강남축구공원 · 3·1운동기념공원 · 남산공원
  VE07: 90,  // 전시시설 — 오죽헌·시립박물관 · 갈골한과체험전시관
  VE12: 90,  // 기타문화관광지 — 한복 문화 창작소
  EX06: 90,  // 산업관광 — 녹색도시체험센터
 ...
 *
 * 체류시간 표와 같은 8개에 숙박 4개를 더한 12행이다. 59행 확장은 중분류
 * 기준표(`LCLS_SYSTM2`)를 기준으로 한다.
 */
