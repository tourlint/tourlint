...
describe('대체 후보의 종류 (#728)', () => {
  ...
  it('🔴 R04 유형 축 — 유형 제한 없이 조회해 다른 관광 유형만 남긴다', async () => {
    const { kto, seen } = listing([
      ...
      // 쇼핑(38)은 마트 · 회센터가 섞여 있어 관광지 자리의 대체로 권하지 않는다 (#748)
      row('5', '38', '300', 'SH01', 'SH010100'),
...
describe('넣을 자리 계산 (R09 ① · R10 · FR-RU-103)', () => {
  ...
  it('🔴 다른 자리가 채운 중분류는 빼고, 남은 것이 없으면 자리를 잡지 않는다 (#589)', () => {
    /*
     * 야간 자리를 먼저 잡는다. 카페가 결손일 때 낮 자리가 먼저 가져가면 18:00 에 들어가
     * 「19:00 이후 없음」 이 남는다. 야간이 카페를 채웠으면 낮 자리는 나머지 결손만 맡는다.
     */
    const a = item({ start: '10:00', end: '11:00' });
    const b = item({ start: '16:00', end: '17:00' });
    expect(planInsertion(r10(['EX02', 'FD05']), [a, b], {}, 90, new Set(['FD05']))?.wantLcls2).toEqual(['EX02']);
    // 비면 「아무 중분류나」 로 읽힌다. 그렇게 넘기지 않고 자리를 접는다
    expect(planInsertion(r10(['FD05']), [a, b], {}, 90, new Set(['FD05']))).toBeNull();
  });
