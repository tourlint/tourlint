 ...
 * → `1.2.3` R07 회사 기준 병기 문장 · R10 표준 프로파일 → `1.2.4` R08 겹친 일정 이동시간 (#554)
 * → `1.2.5` 모르는 것을 정상으로 넘기지 않는다 — R01 「연다」 게이트 · R02 반쪽 기간 · R08 숙박 출발 ·
 *   R09 덜 덮인 예보 · R10 빈 타깃 (#771 · #775 · #776) → `1.2.6` R01 게이트가 AI 가 읽은 조각을 모른다고 하지 않는다 (#784)
 * → `1.2.7` R09 예보를 못 받은 날은 평년 기준으로 판정한다 (#797) → `1.2.8` R05 출발 1일 이내 상품에
 *   「출발 전 운영기관 최종 확인」 을 올린다 (#808).
 ...
 */
export const RULESET_VERSION = '1.2.8';
...
export const RULE_EXPLANATIONS: Readonly<Record<string, RuleExplanation>> = {
  ...
  R05: {
    dataSources: ['KTO'],
    threshold: '어느 관광지인지 정해지지 않았거나 판정에 필요한 정보가 없을 때 · 출발이 1일 안으로 다가왔을 때(감점 없음)',
