...
export const REPORT: readonly Endpoint[] = [
  {
    ...
    description: [
      ...
      '',
      '- **지금 일정의 검수 실행만** 리포트로 만든다 — 검수 이력의 `isCurrent` 인 실행이다. 일정표는 지금 일정을 읽으므로, 다른 실행으로 만들면 그때 판정과 지금 일정이 한 문서에 섞인다 — 409 로 거절하고 다시 검수하도록 안내한다. 수정안을 되돌렸으면 반영 전 실행으로 만든다.',
      ...
      '- 화면은 차단이 0건인 결과에서만 이 버튼을 보인다. 서버는 지금 일정의 실행인지만 확인한다.',
    ].join('\n'),
    ...
    params: {
      runId: { description: '검수 실행 번호. 그 상품의 지금 일정의 실행(`isCurrent`)이어야 한다', example: 111 },
    },
    responses: {
      201: {
    ...
    errors: [
      ...
      },
      {
        ...
        when: '지금 일정의 실행이 아님 — 다시 검수한 뒤 만든다',
        message:
  ...
  },
  {
    ...
    description: [
      ...
      '',
      '- **지금 일정의 검수 결과에 차단이 1건이라도 있으면 403 이다.** 수정안을 되돌렸으면 반영 전 결과로 판정한다 — 반영 후 재검수가 차단 0건이어도 되돌린 일정은 통과하지 못한다.',
      '- 한 번도 검수하지 않은 상품, 수정안을 반영하고 재검수가 끝나기 전인 상품도 403 이다.',
    ...
    errors: [
      {
        ...
        when: '지금 일정의 검수 결과에 차단이 있음(되돌렸으면 반영 전 결과)',
