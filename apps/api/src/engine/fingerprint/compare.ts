 ...
 * ```
 *
 * **비표출 검사가 맨 앞이다** (DR-FP 6-2 · 2026.08.23 개정 · 이슈 #13). `field_names` 불일치를
 * 먼저 보면 판정 필드 목록이 바뀐 실행에서는 비표출 전환이 검사되기 전에 비교 불가로 끝난다.
 * 규칙셋을 고쳐 지문 필드가 늘어난 바로 그 회차에 어떤 관광지가 내려가면 놓치는 것이다.
 *
 * 앞에 둘 수 있는 이유는 두 가지다.
