...
function toEntry(group: HoursGroup): HoursEntry {
   ...
   * 점심 마감을 놓치지만, 두 마감 사이는 준비시간(`breaks`)이 이미 덮으므로 판정 공백이 없다.
   *
   * ⚠️ `docs/파싱규칙.md` §7① 은 `admissionCutoff` 를 `{label, at}` **배열**로 적고 있으나
   *    DR-NM 5-2 · 5-3 의 스키마는 `HH:mm | null` **단일값**이다. 정본(DR-NM)을 따른다.
   */
